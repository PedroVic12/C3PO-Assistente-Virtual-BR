...
class VoiceAssistant:
    ...
    def setup_voice(self):
        ...
        logging.info(f"{Fore.YELLOW}Vozes disponíveis:{Style.RESET_ALL}")
        for i, voice in enumerate(voices):
            logging.info(f"{Fore.YELLOW}{i}: {voice.name}{Style.RESET_ALL}")
        
        self.engine.setProperty('voice', voices[1].id)
        logging.info(f"{Fore.GREEN}Sistema de voz configurado{Style.RESET_ALL}")
    ...
    def speak(self, text: str):
        logging.info(f"{Fore.BLUE}Assistente: {text}{Style.RESET_ALL}")
